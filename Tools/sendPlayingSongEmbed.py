...
async def sendPlayingSongEmbed(self, channel, track):
    ...
    # Embed
    embed = discord.Embed(
        title="Playing Song :", description=f"**[{trackTitle}]({track.uri})**", color=discord.Colour.random()
    )
    if track.thumb:
    ...
    embed.add_field(
        name="Loop :",
        value=isLoop.replace("1", f"{self.bot.emojiList.true}").replace("0", f"{self.bot.emojiList.false}"),
        inline=True,
    )
    embed.add_field(
        name="Loop queue :",
        value=isLoopQueue.replace("1", f"{self.bot.emojiList.true}").replace("0", f"{self.bot.emojiList.false}"),
        inline=True,
    )
